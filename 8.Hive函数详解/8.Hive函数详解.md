...
**返回值**: int  
**说明**: 返回字符串substr在str中首次出现的位置，这里instr输入参数有null会返回null
**举例**:
```sql
...
这些参数需要好好记忆，使用例子如下：
-<span style="color:red"> rows between unbounded preceding and current row</span>（表示从起点到当前行的数据进行，包括当前行）
- <span style="color:red">rows between current row and unbounded following</span>（表示当前行到终点的数据进行，包括当前行）
...
默认框架是 RANGE 而非 ROWS。两者的核心区别在于：
ROWS（物理窗口）：严格按排序后的行号确定窗口范围，与行中的具体值无关。
RANGE（逻辑窗口）：按 ORDER BY 列的值来确定范围，如果排序列有重复值，所有相同值的行会被视为同一范围。</span>

**必须知道：** 三个关键字都可以省略不写，partition by 省略不写，表示不分区。order by 省略不写，表示不排序。rows|range between 开始位置 and 结束位置(窗口帧)，则使用默认值，默认值如下：
over（）里包含order by ，<span style="color:red">默认值为：range between unbounded preceding and current row（表示起点到当前行的数据）。</span>
...
- LEAD(col, n), LAG(col, n) 返回当前记录的下n条或上n条记录的字段值；
